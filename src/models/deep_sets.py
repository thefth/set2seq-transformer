...
class DeepSets(torch.nn.Module):
    def __init__(self, input_dim=24, dim_hidden=256, output_dim=512, 
        ...
        if layer_norm:
            dec_layers.append(torch.nn.LayerNorm(output_dim))
        
        if num_classes:
            dec_layers += [torch.nn.ReLU(),
...
class HierarchicalDeepSets(torch.nn.Module):
    """
    ...
    Attributes:
        ...
        fc (torch.nn.Linear): Final fully connected layer for predictions.
    ...
    def __init__(self, set_input_dim=24, seq_input_dim=512,
        ...
        self.sequence_base = DeepSets(input_dim=seq_input_dim, dim_hidden=dim_hidden, 
                                      pool=pool)
        self.fc = torch.nn.Linear(dim_hidden * 2, num_classes)
