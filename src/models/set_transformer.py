...
class MAB(torch.nn.Module):
    def __init__(self, dim_Q, dim_K, dim_V, num_heads, ln=False):
        ...
        self.ln0 = torch.nn.LayerNorm(dim_V) if ln else None
        self.ln1 = torch.nn.LayerNorm(dim_V) if ln else None
...
class ISAB(torch.nn.Module):
    def __init__(self, dim_in, dim_out, num_heads, num_inds, ln=False):
        ...
        self.I = torch.nn.Parameter(torch.Tensor(1, num_inds, dim_out))
...
class SetTransformer_ISAB_PMA_SAB(torch.nn.Module):
    """
    ...
    Args:
        ...
        set2seq (bool): Whether this is a set-to-sequence transformer.
    """
    def __init__(self, input_dim=24, num_outputs=1, output_dim=512,
                 num_inds=1, dim_hidden=256, num_heads=4,
                 num_classes=2, ln=False, set2seq=False):
        super().__init__()
        self.set2seq = set2seq
        ...
        # Append final layer only if set2seq is False
        if not self.set2seq:
            dec_layers += [torch.nn.Linear(output_dim, num_classes)]

        self.dec = torch.nn.Sequential(*dec_layers)
...
class SetTransformer_ISAB_PMA(torch.nn.Module):
    """
...
class SetTransformer_SAB_PMA(torch.nn.Module):
    """
    ...
    """
    def __init__(self, input_dim=24, num_outputs=1, output_dim=512,
                 dim_hidden=256, num_heads=4, num_classes=2,
                 ln=False, set2seq=False):
        super().__init__()
        self.set2seq = set2seq
...
class HierarchicalSetTransformer(torch.nn.Module):
    """
    ...
    Attributes:
        ...
        fc (torch.nn.Linear): Final fully connected layer for final prediction.
